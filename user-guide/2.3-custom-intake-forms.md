...
description: 'Build custom Project Request intake forms from the Clients hub so clients can submit briefs straight into your Triage queue. Requires a Team plan or higher.'
keywords:
  - ABRAM
  - intake forms
  - project requests
...
Building and publishing custom intake forms ("Project Requests") requires a **Team plan or higher**. The number of forms you can keep **active** at the same time is also capped by plan:
...
Submitted requests land in the **Triage** tab on your Projects page, alongside any projects that do not yet have an owner. That queue is the one place incoming work is listed. From there you can:

* Narrow the queue to just requests, just unowned projects, or both
* See a live count of what is waiting

The **Project Requests** tab in your Inbox shows the same count and takes you straight to the queue. The Triage tab appears for organizations on a Team plan or higher.
...
Requests submitted through a client's portal land in the same Triage queue as everything else, tagged with which client sent them, so you can tell portal submissions apart from public-link submissions at a glance.
