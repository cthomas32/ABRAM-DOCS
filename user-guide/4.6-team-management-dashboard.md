...
* **Calculate Impact**: When you trigger the simulation, the system runs the `simulate_capacity_impact` database function. It recalculates the organization's average utilization, highlights the projected overbooked count, and generates dynamic alerts/warnings.
* **Visual Warnings**: Sibling tables and charts highlight affected crew members. Color coding reflects capacity load:
  * 🟡 **Amber Highlight**: The member's projected utilization falls between 90% and 99% (near capacity).
  * 🔴 **Red Highlight**: The member's projected utilization is 100% or greater (overcommitted).
