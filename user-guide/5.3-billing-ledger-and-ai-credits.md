---
title: "Billing ledger and AI credits"
description: "Understand how ABRAM meters AI credits per action, structures the organization-bound ledger, and gates producer features based on credit balances."
---

# Section 5.3: Billing Ledger and AI Credits

This guide explains how AI credits are calculated, structured, and consumed in the ABRAM Network. It details how the organization-bound ledger works and how producer features are gated based on credit balances.

---

## 1. The AI Credit Concept

ABRAM meters and bills all user-initiated AI actions—such as parsing resumes, extracting project skills, matching candidates, or analyzing project briefs.

---

## 2. Typical Credit Costs & Caching

Credit consumption is calculated based on the complexity of the operation, length of text processed, or flat service rates. This ensures that you only pay for the resources required to process your request.

| Feature / Action | Service Engine | Billing Model | Typical Cost / Range |
| :-- | :-- | :-- | :-- |
| **Project Brief Analysis** | Advanced Analysis | Based on brief size & details | 5 - 25 credits per analysis |
| **AI Team Matchmaking Suggestions** | Advanced Analysis | Based on scope size & candidate count | 5 - 20 credits per suggestion |
| **Resume & Profile Importing** | Standard Estimation | Based on resume file size | 2 - 10 credits per import |
| **Chatbot Co-pilot Interaction** | Standard / Advanced | Per query/response | 0.5 - 5 credits per message |
| **Web Search Tool** | Integration Feature | Flat rate per search | 2 credits per search |
| **Image Generation** | Creative Tool | Flat rate per generation | 10 credits per image |

> \[!NOTE\] **Prompt Caching** automatically optimizes credit usage. If you repeat similar queries or ask follow-up questions within the same context (e.g. refining a project brief or candidates list), the system reads from the cache at a fraction of the standard credit cost, saving your organization credits.

---

## 3. The Three-Pool Credit Structure

Every organization's ledger is divided into three distinct credit pools. When credits are deducted, they are drawn in a strict priority order:

```text
  [1] Monthly Allowance  ──►  [2] Trial Credits  ──►  [3] Purchased Balance
  (Resets monthly)           (Valid until expiry)     (Never expires)
```

1. **Monthly Allowance**: Included in your subscription tier (e.g., Team, Studio). Drawn first. It reset every month; unused allowance does not roll over.
2. **Trial Credits**: Awarded during onboarding. Drawn second. Only valid if the trial is still active and has not expired.
3. **Purchased Balance**: Top-up credits bought via Stripe. Drawn last. Purchased credits never expire.

---

## 4. Organization-Bound Billing

- **Organization-Bound Billing**: All billing ledgers are bound to organizations. Solo users are billed through their personal organization workspace.
- **Membership Routing**: If a user belongs to an active organization membership, the organization's ledger is billed.
- **Onboarding Exception**: AI calls made during the onboarding wizard (e.g., parsing your initial resume when setting up your profile) are free and bypass the ledger.

---

## 5. Credit Gating (Example: Project Brief Analysis)

When a producer uploads a project brief for AI analysis, the platform enforces the following sequence:

```mermaid
sequenceDiagram
    participant User as Producer Interface
    participant App as Platform Server
    participant Ledger as Billing Ledger
    participant AI as AI Engine
    
    User->>App: Request Brief Analysis
    App->>Ledger: Resolve Billing Entity (User or Org)
    App->>Ledger: Check Credit Balance
    alt Balance is 0 or less
        Ledger-->>App: Insufficient Credits
        App-->>User: Show Payment Required / Credit Recharge
    else Balance is positive
        App->>AI: Invoke Analysis
        AI-->>App: Return Analysis
        App->>Ledger: Deduct Credits for usage
        Ledger-->>App: Deduct & Log transaction
        App-->>User: Return completed Analysis
    end
```

### Disconnect Safety Net

If a network disconnect or timeout occurs during an analysis, the platform ensures your ledger is only billed for the work actually completed.

---

## 6. Upgrading Plans and Buying Credits

Owners and Admins can purchase additional credits or upgrade plan tiers in **Settings** -\> **Billing**:

### Buying Ad-Hoc Top-Ups

If your balance runs low, you can click **Top-Up** to purchase credit packs (e.g., \$10.00 for 1,000 credits). This opens Stripe Checkout to securely process the purchase, and updates your balance immediately.

### Upgrading a Personal Workspace

If you are currently on the **Free** tier in a personal workspace and select a team subscription (e.g., **Team** or **Studio**):

1. **Workspace Promotion**: The platform launches a coordinated flow prompting you for your Company Name and Team Size.
2. The platform automatically promotes your personal workspace to a full **Organization**.
3. You are redirected to Stripe Checkout to set up the subscription.
4. Once completed, your organization's Monthly Allowance is active, and team seat limits are updated.