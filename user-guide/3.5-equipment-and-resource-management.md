...
* **Bulk Import**: Click the **Import** button. You can upload a CSV/TSV/TXT file or directly paste tab-separated spreadsheet data. 
  > [!NOTE]
  > The system does not currently offer a downloadable spreadsheet file template. Instead, a column formatting helper is displayed in the modal interface showing the expected format: `Name, Type, Quantity, Serial Number, Barcode, Condition, Daily Rate, Identifier`. Only `Name` is required.
...
* **Needs Repair Lockout (Planned Feature)**: Damaged or needs maintenance items derive their status as Blocked or Maintenance in the frontend interface. Direct scheduling lockouts preventing conflicting allocations are in development.
