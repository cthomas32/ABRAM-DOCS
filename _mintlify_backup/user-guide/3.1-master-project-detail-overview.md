---
title: "Master project detail page overview"
description: "Tour the ABRAM project workspace—header, info strip, and Overview, Tasks, Timeline, Team, Financial, and Documents tabs for daily project tracking."
---
...
```
+--------------------------------------------------------------+
| [Back]  Project Title  [Progress: 68%]  [Status: Planning]   | <-- Compact Header
+--------------------------------------------------------------+
| Industry: Commercial | Budget: $25k | Dates: Jun 15 - Jul 30 | <-- Info Strip
+--------------------------------------------------------------+
| [Overview] [Tasks] [Timeline] [Team] [Financial] [Documents] | <-- Minimal Tab Nav
+--------------------------------------------------------------+
|                                                              |
|                     Active Tab Content                       |
|                                                              |
...
* Your tab preferences are saved in your browser's local storage per project and user.
...
### Tasks Tab
The task and scope manager. Features a centralized workspace to view, edit, and organize deliverables, milestones, and work orders. (See [Section 3.4](/user-guide/3.4-task-lists-and-tracking) for details).
...
* **Active Tab**: Sharing a link ending in `?tab=tasks` opens the project page with the Tasks tab pre-selected.
* **Direct Item View**: Sharing a link ending in `?workOrderId=your-id-here` automatically loads the project view and opens the configuration details for that specific Work Order.
* **Validations**: If a shared link contains an expired or invalid ID, the page automatically updates the URL and reverts safely to the default Overview tab to prevent errors.
...
3. **Customizing Visible Tabs**:
   ...
   * Click **Save Layout**. The tab list updates immediately, and your settings are saved in local storage.
