...
-- ---------------------------------------------------------------------
DROP POLICY IF EXISTS "Growth reads subscribers" ON public.subscribers;
CREATE POLICY "Growth reads subscribers"
    ON public.subscribers FOR SELECT TO authenticated
    USING (public.is_growth_member());

DROP POLICY IF EXISTS "Growth reads campaign logs" ON public.campaign_logs;
CREATE POLICY "Growth reads campaign logs"
    ON public.campaign_logs FOR SELECT TO authenticated
    USING (public.is_growth_member());

DROP POLICY IF EXISTS "Growth reads email templates" ON public.email_templates;
CREATE POLICY "Growth reads email templates"
...
-- USING says which rows may be edited: drafts only, so a sent campaign
-- becomes read-only history the moment it leaves. WITH CHECK says what
-- they may become: still a draft. Both halves are needed — USING alone
-- would let a draft be updated straight to 'sending'.
