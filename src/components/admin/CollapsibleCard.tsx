  ...
  return (
    <div className="glass-panel rounded-2xl border-white/8">
      ...
      >
        <span className="min-w-0">
          <span className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold uppercase tracking-[0.15em] text-zinc-300 truncate">
              {title}
            </span>
            {badge && (
              <span className="text-[9px] font-mono text-zinc-500 bg-white/[0.04] border border-white/8 px-1.5 py-0.5 rounded shrink-0">
                {badge}
              </span>
            )}
          </span>
          {subtitle && <span className="block text-[10px] text-zinc-500 mt-1 truncate">{subtitle}</span>}
        </span>
        <ChevronDown
      ...
      <div className="hidden md:block px-6 pt-6">
        <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-500 font-sans flex items-center gap-2">
