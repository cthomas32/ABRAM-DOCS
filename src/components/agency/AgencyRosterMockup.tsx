    ...
    <div className="w-full max-w-5xl mx-auto rounded-2xl border border-white/5 bg-zinc-950/40 p-4 sm:p-6 lg:p-8 backdrop-blur-xl relative overflow-visible">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4 mb-6">
        ...
        <button
          ...
          className="btn-glass rounded-full px-4 py-2 text-xs inline-flex items-center gap-1.5 shrink-0 justify-center cursor-pointer font-semibold min-h-[44px]"
        >
          {isOptimizing ? (
            <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="lg:col-span-8 flex flex-col space-y-4">
          <div className="hidden md:flex justify-between items-center px-1 gap-2">
            <span className="text-[10px] font-semibold tracking-wider text-zinc-500 uppercase font-sans truncate">
              Roster database ({filteredRoster.length} matches)
            </span>
            <span className="text-[9px] text-zinc-400 font-sans shrink-0">
              Click ROW to shortlist crew asset
            </span>
          </div>

          <div className="flex items-center justify-between mb-2 md:hidden px-1 gap-2">
            <span className="text-[10px] text-zinc-500 uppercase font-semibold truncate">
              Roster database ({filteredRoster.length} matches)
            </span>
            <span className="text-[10px] text-zinc-400 font-sans shrink-0">Swipe to view →</span>
          </div>
          ...
          <div className="border border-white/5 rounded-xl bg-zinc-950/30 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left text-xs min-w-[700px]">
                <thead>
                  <tr className="border-b border-white/5 bg-zinc-950/40 text-[9px] uppercase tracking-wider font-semibold text-zinc-500 font-sans">
                ...
                <tbody className="divide-y divide-white/5 font-sans text-zinc-300">
                  {filteredRoster.map((m) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="font-semibold text-white flex items-center gap-1.5">
                              {m.name}
                              {m.isShortlisted && (
                                <span className="text-[8px] bg-white/[0.10] text-white border border-white/[0.20] font-sans font-bold px-1 py-0.2 rounded uppercase">
                                  Shortlisted
                                </span>
                              )}
                            </div>
                      ...
                      <td className="px-4 py-3 text-zinc-400 font-sans text-[10px]">{m.dept}</td>
                      ...
                      <td className="px-4 py-3 font-sans text-white font-semibold">
                        ${m.rate}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-block px-2 py-0.5 rounded text-[9px] font-sans font-medium ${
                          m.status === "Available"
            ...
            {isOptimizing ? (
              <div className="h-48 flex flex-col items-center justify-center text-center space-y-3">
                ...
                <div className="space-y-1">
                  <p className="text-xs text-white font-semibold font-sans">AI Roster Matcher Running</p>
                  <p className="text-[10px] text-zinc-500">{optimizedText}</p>
                </div>
              </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2.5 text-xs">
                    <div className="p-3 rounded-lg border border-white/5 bg-zinc-950/40 space-y-1">
                      <span className="text-[9px] uppercase tracking-wider font-sans text-zinc-500 block">Total Roster Rate</span>
                    ...
                    <div className="p-3 rounded-lg border border-white/5 bg-zinc-950/40 space-y-1">
                      <span className="text-[9px] uppercase tracking-wider font-sans text-zinc-500 block">Avg. Match Score</span>
                  ...
                  <div className="space-y-2">
                    <span className="text-[9px] uppercase font-sans tracking-wider text-zinc-500 block">Shortlisted Assets</span>
                      ...
                      {shortlistedCrew.map((c) => (
                        <div key={c.id} className="flex items-center justify-between p-2 rounded bg-zinc-950/60 border border-white/5">
                          ...
                          <div className="text-right shrink-0">
                            <span className="text-white font-sans font-semibold block">${c.rate}/d</span>
                            <span className="text-[9px] font-sans text-emerald-400 font-medium">{c.suitability}% Match</span>
                          </div>
                        </div>
                      ))}
                    </div>
