...
import { SegmentedSwitch } from "@/components/admin/SegmentedSwitch";
...
export default function PeopleWorkspace({ tab }: { tab: PeopleTab }) {
  ...
  useEffect(() => {
    ...
    if (wanted && contacts.some((contact) => contact.id === wanted)) {
      // A person has their own address now. The query form forwards to it
      // and replaces, so it does not sit in the back stack.
      router.replace(`/admin/dashboard/people/${wanted}`);
    }
  }, [deepLinkDone, loading, contacts, router]);
  ...
  const captureHref = profile
    ? `/admin/dashboard/capture?p=${encodeURIComponent(profile.slug)}`
  ...
  /**
   * A built-in list, opened straight from the address.
   *
   * `?list=subscribers` is what the mailing list is now: there is no
   * subscribers screen any more, so anything that used to link to one
   * links here. Deliberately second to `?view=`, because a saved list is
   * somebody's own question and a built-in one is everybody's.
   *
   * No wait on loaded rows, unlike the view effect above: a smart list is
   * code, so it can be applied before anything has arrived.
   */
  const [smartLinkDone, setSmartLinkDone] = useState(false);
  useEffect(() => {
    if (smartLinkDone) return;
    setSmartLinkDone(true);
    const params = new URLSearchParams(window.location.search);
    if (params.get("view")) return;
    const wanted = params.get("list");
    if (wanted && smartList(wanted)) applySmartList(wanted);
  }, [smartLinkDone, applySmartList]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="hidden sm:block text-xs text-zinc-400 mt-1 font-sans max-w-2xl leading-relaxed">
              Everybody who has reached us, however they arrived: the mailing list, an app
              application, a form, an import, or a code scanned across a table. One row per person,
              with every way in recorded on it.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            {view === "table" ? (
              <PeopleTable
                ...
                onOpen={(contact) => router.push(`/admin/dashboard/people/${contact.id}`)}
                canWrite={canWrite}
