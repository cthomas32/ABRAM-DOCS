...
/* Lists are a tab on People rather than a route of their own, so there is
   one path to revalidate. `/admin/dashboard/lists` was in here until the
   address scheme changed; it is a redirect now, and revalidating a
   redirect refreshes nothing. */
const PATHS = ["/admin/dashboard/people"];
